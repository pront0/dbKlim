<body bgcolor="#CCCCCC">

<h2><p align="center">ADD DATA</p></h2>

<form method="post" action="action_RCP45.php">

<table width="546" border="0" cellpadding="0" cellspacing="0" align="center" bgcolor="#FFFFFF">

<tr>
  <td width="189" height="20"> </td>

  <td width="26"> </td>

  <td width="331"> </td>

</tr>

<tr>

  <td height="27" align="right" valign="middle">Tahun</td>

  <td align="center" valign="top">:</td>

  <td valign="middle"><label>

    <input name="tah" type="text" size="10">

  </label></td>

</tr>

<tr>

  <td height="27" align="right" valign="middle">Januari</td>

  <td align="center" valign="top">:</td>

  <td valign="middle"><label>

    <input name="jan" type="text" size="5">

  </label></td>

</tr>

<tr>

  <td height="27" align="right" valign="middle">Februari</td>

  <td align="center" valign="top">:</td>

  <td valign="middle"><label>

    <input type="text" name="feb" size="5">

  </label></td>

</tr>

<tr>

  <td height="27" align="right" valign="middle">Maret</td>

  <td align="center" valign="top">:</td>

  <td valign="middle"><label>

    <input type="text" name="mar" size="5">

  </label></td>

</tr>

<tr>

  <td height="27" align="right" valign="middle">April</td>

  <td align="center" valign="top">:</td>

  <td valign="middle"><label>

    <input type="text" name="apr" size="5">

  </label></td>

</tr>

<tr>

  <td height="27" align="right" valign="middle">Mei</td>

  <td align="center" valign="top">:</td>

  <td valign="middle"><label>

    <input type="text" name="mei" size="5">

  </label></td>

</tr>

<tr>

  <td height="27" align="right" valign="middle">Juni</td>

  <td align="center" valign="top">:</td>

  <td valign="middle"><label>

    <input type="text" name="jun" size="5">

  </label></td>

</tr>

<tr>

  <td height="27" align="right" valign="middle">Juli</td>

  <td align="center" valign="top">:</td>

  <td valign="middle"><label>

    <input type="text" name="jul" size="5">

  </label></td>

</tr>

<tr>

  <td height="27" align="right" valign="middle">Agustus</td>

  <td align="center" valign="top">:</td>

  <td valign="middle"><label>

    <input type="text" name="agu" size="5">

  </label></td>

</tr>

<tr>

  <td height="27" align="right" valign="middle">September</td>

  <td align="center" valign="top">:</td>

  <td valign="middle"><label>

    <input type="text" name="sep" size="5">

  </label></td>

</tr>

<tr>

  <td height="27" align="right" valign="middle">Oktober</td>

  <td align="center" valign="top">:</td>

  <td valign="middle"><label>

    <input type="text" name="okt" size="5">

  </label></td>

</tr>

<tr>

  <td height="27" align="right" valign="middle">November</td>

  <td align="center" valign="top">:</td>

  <td valign="middle"><label>

    <input type="text" name="nov" size="5">

  </label></td>

</tr>

<tr>

  <td height="27" align="right" valign="middle">Desember</td>

  <td align="center" valign="top">:</td>

  <td valign="middle"><label>

    <input type="text" name="des" size="5">

  </label></td>

</tr>


<tr>

  <td height="42"> </td>

  <td> </td>

  <td><input type="submit" name="ttambah" value"TAMBAH"></td>

</tr>

</table>

</form>

</body>
